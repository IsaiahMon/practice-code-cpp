...
void read(ifstream& ifs, vector<Noble*>& nobles, vector<Warrior*>& freeWarriors){
  ...
  while(ifs >> command){ //Read first command to determine next course of action
	...
	cerr << "Hiring a Warrior requires name of both the Noble and Warrior, respectively."
	     <<endl;
	exit(1);
      }
    }
	...
	cerr << "Firing a Warrior requires name of both the Noble and Warrior, respectively."
	     <<endl;
	exit(1);
      }
    }
